...
IF NOT EXISTS `backon_interface` (
	...
	`pre_interceptor` text COMMENT '接口调用前的拦截器的配置，使用JsonArray格式存储,默认为空',
  `post_interceptor` text COMMENT '接口调用后的拦截器的配置，使用JsonArray格式存储,默认为空',
	`is_deleted` tinyint(4) unsigned NOT NULL DEFAULT '0' COMMENT '逻辑删除标志位，1表示已删除，0表示未删除',
	`create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
	`update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
	`create_by` varchar(40) DEFAULT NULL,
...
IF NOT EXISTS `gateway_interface` (
	...
	`name` varchar(255) NOT NULL,
	`url` varchar(255) NOT NULL DEFAULT '',
	`backon_url` text NOT NULL DEFAULT '[]' COMMENT '该接口需要调用的后台系统接口,并发和复杂类型用jsonArray的存储',
